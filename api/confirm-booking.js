// api/confirm-booking.js — 藤沢鵠沼 Hideaway
...
<div style="font-family:'Helvetica Neue',sans-serif;max-width:560px;margin:0 auto;color:#1A1A14">
  <div style="background:#0A3D2E;padding:28px 32px">
    <h1 style="color:#fff;font-size:22px;font-weight:300;margin:0">藤沢鵠沼 <em style="color:#C8A96E">Hideaway</em></h1>
  ...
  <div style="background:#0A3D2E;padding:16px 32px;text-align:center">
    <p style="color:rgba(255,255,255,0.4);font-size:11px;margin:0">© 2025 藤沢鵠沼 Hideaway · Powered by Stripe</p>
  </div>
</div>`;

  const hostHtml = `
<div style="font-family:'Helvetica Neue',sans-serif;max-width:520px;color:#1A1A14">
  <h2 style="color:#0A3D2E">🌊 新しいご予約 — 藤沢鵠沼 Hideaway</h2>
  ...
  try {
    ...
    const [guestResult, hostResult] = await Promise.allSettled([
      resend.emails.send({
        ...
        to:      guestEmail,
        subject: `【予約確認】藤沢鵠沼 Hideaway ${checkIn}〜${checkOut}`,
        html:    guestHtml,
      }),
      resend.emails.send({
        from:    `Kugenuma Booking <${fromAddr}>`,
        to:      hostEmail,
        subject: `[新予約] 藤沢鵠沼 Hideaway — ${guestName || guestEmail} ${checkIn}`,
        html:    hostHtml,
      }),
    ]);
